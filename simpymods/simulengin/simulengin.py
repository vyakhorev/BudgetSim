...
if __name__ == '__main__':
    # TODO !!!
    # Test cContainerPart
    def supply(env, cont, qtty, freq):
        while 1:
            cont.put(qtty)
            print("Supplied " + str(qtty))
            yield env.timeout(freq)

    def demand(env, cont, qtty, freq):
        while 1:
            yield env.process(cont.get_available(qtty))
            print("Got " + str(qtty)) #Wrong
            yield env.timeout(freq)

    env = simpy.Environment()
    cont = cContainerPart(env)
    env.process(supply(env, cont, 1000, 10))
    env.process(demand(env, cont, 500, 3))
    env.run(until = 100)